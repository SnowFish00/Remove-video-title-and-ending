    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for folder_name in os.listdir(input_video_dir):
            ...
            if os.path.isdir(folder_path):
                cut_folder_path = os.path.join(cut_image_dir, folder_name)
                output_folder_path = os.path.join(
                    output_video_dir, folder_name)
                if not os.path.exists(output_folder_path):
                    os.makedirs(output_folder_path)

                for video_file in os.listdir(folder_path):
                    if video_file.lower().endswith('.mp4'):
                        executor.submit(process_video_concurrently, video_file,
                                        folder_path, cut_folder_path, output_folder_path)
